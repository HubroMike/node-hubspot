...
class Company {
  ...
  getByProps(searchProps, recursive = true, returnProps = [], offset = 0) {
    ...
    return this.get({ properties: [...returnProps, ...Object.keys(searchProps)], offset }).then(data => {
        ...
        _.forEach(data.companies, company => {
          ...
          _.forEach(Object.keys(searchProps), key => {
            if(company.properties[key] && _.lowerCase(company.properties[key].value) === _.lowerCase(searchProps[key])) {
              // console.log("found a matching company!!");
              searchScore++;
            }
          });
          if(searchScore !== 0) results.push({ company, searchScore });
        });

        if( results.length !== 0 ) {
          return _.sortBy(results, ["searchScore"]);
        }
