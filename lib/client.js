...
class Client extends EventEmitter {
  ...
  _request(opts, skipUsageCheck = false) {
    const params = prepareParams(opts, this)
    return this.checkApiLimit(params, skipUsageCheck).then(() => {
  ...
  timeoutRetryRequest(opts, retries = 10, minTimeout = 5000) {
    return promiseRetry(
      (retry, number) => {
        debug('attempt number', number)
        return this._request(opts).catch(error => {
          switch (error.code) {
            case 'ECONNRESET':
  ...
  checkApiLimit(params, skipUsageCheck = false) {
    if (skipUsageCheck) {
      return Promise.resolve()
    }
    return new Promise((resolve, reject) => {
      ...
      if (this.currentUsage !== undefined) {
        ...
        if (usagePercent > this.maxUsePercent) {
          // check if the usage stats hasn't reset yet
          return this.getApiLimit(true).then(data => {
            // if the usage still exceeds, reject the call
            const usagePercent = (100.0 * this.currentUsage) / this.usageLimit
            if (usagePercent > this.maxUsePercent) {
  ...
  getApiLimit(skipUsageCheck = false) {
    ...
    return this._request(
      ...
      this.currentUsage = this.limit.currentUsage
      this.usageLimit = this.limit.usageLimit
      return this.limit
    })
  }
}
